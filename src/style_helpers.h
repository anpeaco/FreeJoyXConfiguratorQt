...
// tipHtml(...) lives here -- the canonical tooltip markup builders, plus the
// runtime formatTip() chokepoint. Kept in a Qt-Core-only header so they stay
// unit-testable without widgets; re-exported here for existing call sites.
...
// tipHtml(heading, points) / tipHtml(heading, body) moved to tip_format.h
// (included above) so the formatter that shares them stays Qt-Core-only and
// headlessly testable. Prefer authoring plain tooltips per the convention there
// -- the tooltip engine runs them through formatTip() at display time -- rather
// than calling these builders directly. They remain for dynamic, code-built tips.
