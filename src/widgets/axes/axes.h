...
public:
     ...
     * push the rotated pin's source onto the armed axis row.
    ...
    /* Toggle this axis's "Detect" button between armed and idle, with
     * matching label change ("Detect" <-> "Cancel"). Used by AxesConfig
     * to (a) restore the visual state after a successful detection,
