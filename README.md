...
* **GPIO expander configuration (MCP23017 I2C / MCP23S17 SPI)** — on the Shift Registers tab, up to 8 chips in any mix, each with a Type selector (Disabled / MCP23017 I2C / MCP23S17 SPI), button count (16 max), internal pull-ups, and invert. I2C chips pick an address `0x20`–`0x27` (duplicate clashes flagged); SPI chips take a CS pin via the *MCP23S17 expander CS* role in the Pin Config dropdown (CS pins map to SPI slots in order). Validation warns when the I2C/SPI bus isn't enabled or there are too few CS pins. The expander buttons surface in the Buttons-tab Physical panel as *Expander N*, mappable with every Function type
...
### Unreleased
- **Reinstall firmware button (Flasher tab)** — force-flash the current firmware even when the device already reports the latest version, closing the gap where the device-card *Upgrade firmware* button greys out on a same-version board. Reuses the normal backup → flash → restore flow (see Features).
- **GPIO expander configuration** — MCP23017 (I2C) / MCP23S17 (SPI) button expanders on the Shift Registers tab (wire generation `0x0030`); see Features. Reads and migrates `0x0020` configs forward.

### v0.2.0
- **Host HID-number badges, reorder fix, and mute-in-place display** — buttons show their host HID number; a disabled button is muted in place (keeps its number and reports 0) rather than renumbering the rest; assorted reorder-tracking and tooltip fixes. Pairs with firmware **v0.2.0** (F411 clock self-sufficiency + USB hardening). No wire-format change.
...
* **FreeJoyX v0.1.x / v0.2.0** (`0x0020`) — the pre-expander generation. `0x0030` appended `gpio_expanders[]` + `saved_per_exp[]` at the end of `dev_config_t`, so `0x0020` is the byte-exact prefix of the current shape; the migrator prefix-copies and zero-fills the new arrays.
* **FreeJoyX v0.0.x** (`0x0010`) — first generation after the upstream-lineage reset. Shares the `0x0020` shape; the `0x0010`→`0x0020` bump was a semantic change only (LONG_PRESS enum slot reinterpreted as TAP). Migrator is a prefix-copy + version restamp; the configurator logs the gesture-semantic shift so the user re-verifies any gesture-typed buttons on the migrated config.
