...
* **Logic button configuration** — pick `Logic` in the Function dropdown to unlock Operator (`AND`, `OR`, `NAND`, `NOR`, `XOR`, `XNOR`) and Source B cells. Logic debounce is a single global setting shared by every logic button and shift, on the **Timers** tab (no longer per-slot). (`NOT` and `A AND NOT B` exist in the wire-format enum for back-compat with shipped configs but are no longer offered in the picker.)
...
* **Shifts tab** — the eight shift modifiers are configured *like buttons* (physical input, and a type: **Normal** momentary, **Toggle** for a latching shift-lock, **Logic** to combine two inputs, or **Sequential toggle** to cycle shift layers with one button — press advances S1 → S2 → … → wraps) on their own tab, writing a dedicated `shift_buttons[]` array (wire gen `0x0070`). Shift buttons never appear as joystick buttons and no longer consume one of the 128 button slots. Only *held*-state types are offered (the momentary Toggle-switch / gesture / POV / encoder types are excluded), and the per-slot Delay/Press columns are hidden for shifts. The global timers (Timer 1/2/3, Debounce, Logic debounce, Tap cutoff, Double tap cutoff; the last two default 200 ms) live on the adjacent **Timers** tab.
