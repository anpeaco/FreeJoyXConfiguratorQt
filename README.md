...
* **Shifts tab** — the eight shift modifiers are configured *like buttons* (physical input, and a type: **Normal** momentary, **Toggle** for a latching shift-lock, or **Logic** to combine two inputs) on their own tab, writing a dedicated `shift_buttons[]` array (wire gen `0x0060`). Shift buttons never appear as joystick buttons and no longer consume one of the 128 button slots. The global timers (Timer 1/2/3, Debounce, Tap cutoff, Double tap cutoff; the last two default 200 ms) live on the adjacent **Timers** tab.
