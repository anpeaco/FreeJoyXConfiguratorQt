...
* **Shift register configuration** — the Data / CLK / Latch columns are per-register *Auto / \<pin\>* dropdowns. **Auto** (the default) keeps the legacy positional mapping — the k-th role pin drives register k — so existing configs behave identically. Picking a specific pin overrides that register's line, letting a multi-chain build choose which physical pin is which register (e.g. to fix button-number ordering or drive separate latch/clk buses). The pick packs into the register's reserved config nibbles with no wire-format bump; an all-Auto config writes zeros, matching older firmware
